"""
backtest_v6_n200c.py - V6(N200) + 2 clean fixes.

Fixes from N200 analysis (position cap dropped - wrong approach):
  Fix 1: OVERSOLD_EXHAUSTION restricted to Nifty 50 stocks only
          Midcap OE: 50% WR, Rs -225 avg = noise
          Nifty50 OE: 63% WR, Rs +408 avg = signal
  Fix 2: Remove 4 weak tickers:
          IRFC (50% WR), RAILTEL (43% WR),
          M&MFIN (44% WR), LICHSGFIN (48% WR)

Position cap: NOT applied - ticker-first loop makes it incorrect
Drawdown 8.1% accepted - recover in 2.4 months at current avg

V6(N200) baseline: Rs 20,013/mo, 55.5% WR, 8.1% dd, 1.37x pf
...
def run_backtest():
    ...
    outfile=f"backtest_v6n200c_{ts}.csv"
...
def _print_results(df):
    print("\n"+"="*60)
    print("  BACKTEST V6 N200c RESULTS")
    print("="*60)
    ...
    print(f"\n  COMPARISON")
    print(f"  {'Metric':<22} {'V6(45)':>8} {'N200':>8} {'N200c':>8}")
    print(f"  {'-'*56}")
    print(f"  {'Win Rate':<22} {'57.8%':>8} {'55.5%':>8} {wr:.1f}%")
    print(f"  {'Monthly avg':<22} {'9,203':>8} {'20,013':>8} {tot/(yrs*12):,.0f}")
    print(f"  {'Annual return':<22} {'18.4%':>8} {'40.0%':>8} {tot/CAPITAL/yrs*100:.1f}%")
    print(f"  {'Max drawdown':<22} {'4.5%':>8} {'8.1%':>8} {mdd*100:.1f}%")
    print(f"  {'Profit factor':<22} {'1.48x':>8} {'1.37x':>8} {pf:.2f}x")
    ...
    if tot/(yrs*12)<18000: invalid.append(f"Monthly Rs {tot/(yrs*12):,.0f} < Rs 18,000 minimum")
