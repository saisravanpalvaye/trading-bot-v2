"""
backtest_v6.py - V4 + 3 fixes + wider SL/TGT.

Changes from V4:
  1. Sector proxies: CHEMICAL->CNXFMCG, CONSUMER->CNXCONSUMP
  2. Duplicate position block in backtest loop
  3. ATR_SL_MULT 1.5->2.0, ATR_TGT_MULT 2.5->3.0 (RR math forced)

RS filter: DROPPED (removed 54% WR trades in V5 - harmful)
All other V4 logic preserved exactly.

V6 invalidated if: WR<54.6%, monthly<9538, dd>8%, pf<1.45x
"""
...
def _print_results(df):
    ...
    print(f"\n  COMPARISON")
    print(f"  {'Metric':<22} {'Old':>7} {'V4':>7} {'V5':>7} {'V6':>7}")
    print(f"  {'-'*56}")
    print(f"  {'Win Rate':<22} {'52.1%':>7} {'54.6%':>7} {'54.6%':>7} {wr:.1f}%")
    print(f"  {'Monthly avg':<22} {'6,090':>7} {'9,538':>7} {'7,098':>7} {tot/(yrs*12):,.0f}")
    print(f"  {'Annual return':<22} {'18.4%':>7} {'19.1%':>7} {'14.2%':>7} {tot/CAPITAL/yrs*100:.1f}%")
    print(f"  {'Max drawdown':<22} {'4.2%':>7} {'5.2%':>7} {'5.8%':>7} {mdd*100:.1f}%")
    print(f"  {'Profit factor':<22} {'1.96x':>7} {'1.45x':>7} {'1.41x':>7} {pf:.2f}x")
