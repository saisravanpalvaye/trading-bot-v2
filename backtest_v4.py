"""
backtest_v4.py — Improved system based on data evidence.

Changes from current system:
  1. OVERSOLD_EXHAUSTION RSI: 50 → 38
  2. TELECOM removed (29% WR), HOSPITAL removed (41% WR)
  3. BREAKOUT_PULLBACK added (56% WR proven in v3 backtest)
...
Compare vs:
  ...
  V3 new: 51.8% WR, Rs 5,781/mo, 11.6% ann, 12.0% dd, 1.24x pf
"""
...
def run_backtest():
    print("\n"+"="*60)
    print("  BACKTEST V4 — Improved System")
...
def _print_results(df):
    ...
    print(f"\n  COMPARISON")
    print(f"  {'Metric':<22} {'Old':>10} {'V3':>10} {'V4':>10}")
    print(f"  {'-'*54}")
    print(f"  {'Win Rate':<22} {'52.1%':>10} {'51.8%':>10} {wr:.1f}%")
    print(f"  {'Monthly avg':<22} {'Rs 6,090':>10} {'Rs 5,781':>10} Rs {tot/(yrs*12):,.0f}")
    print(f"  {'Annual return':<22} {'18.4%':>10} {'11.6%':>10} {tot/CAPITAL/yrs*100:.1f}%")
    print(f"  {'Max drawdown':<22} {'4.2%':>10} {'12.0%':>10} {mdd*100:.1f}%")
    print(f"  {'Profit factor':<22} {'1.96x':>10} {'1.24x':>10} {pf:.2f}x")
    ...
    beats=sum([wr>52.1, tot/(yrs*12)>6090, mdd*100<4.2, pf>1.96])
    print(f"\n  VERDICT: Beats old on {beats}/4 metrics")
    if beats>=3: print("  → V4 is better — proceed with build")
    elif beats==2: print("  → Mixed — review setup breakdown before deciding")
