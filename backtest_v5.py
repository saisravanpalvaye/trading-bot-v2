"""
backtest_v5.py — V4 + Relative Strength filter.

V4 changes (all preserved exactly):
  1. OVERSOLD_EXHAUSTION RSI: 50 → 38
  2. TELECOM removed (29% WR), HOSPITAL removed (41% WR)
  3. BREAKOUT_PULLBACK added (56% WR proven)
  4. Partial profit at 2x ATR
  5. Sector confirmation hard block
  6. Fixed fractional risk sizing (Rs 6,000 per trade)
  7. Hold days 6 → 8

V5 single addition (data-driven, academically proven):
  8. Relative Strength filter on TREND_PULLBACK + BREAKOUT_PULLBACK
     RS = stock 63-day return / sector 63-day return
     RS must be > 1.0 (stock outperforming sector)
     NOT applied to RSI_DIVERGENCE or OVERSOLD_EXHAUSTION
     (reversal setups — stock may be underperforming because it's
      been sold off. RS gate would block the best reversal entries.)
     Safe default: if sector data missing → allow signal

Test cases: 28/28 passed before build.
Mechanism: institutional accumulation pushes stock above sector peers.
This is the momentum factor — proven across 150 years, 46 countries.
NOT seasonal gating, NOT curve fitting.

Compare vs:
  Old: 52.1% WR, Rs 6,090/mo, 18.4% ann, 4.2% dd, 1.96x pf
  V4:  54.6% WR, Rs 9,538/mo, 19.1% ann, 5.2% dd, 1.45x pf

V5 expected: higher WR, higher avg P&L, better profit factor
Invalidated if: WR < 54.6%, monthly < Rs 9,538, drawdown > 8%,
                or fewer than 50 trades/year
"""
...
SECTOR_PROXY = {
    ...
    "CAPITAL":"^CNXINFRA","CHEMICAL":"^CNXINFRA","CONSUMER":"^CNXFMCG",
...
# ── Relative Strength filter (V5) ─────────────────────────
RS_LOOKBACK = 63  # 3 months — captures real momentum, not noise

def _relative_strength(stock_df, sector_df, dt):
    """
    RS = (stock 63-day return) / (sector 63-day return)
    RS > 1.0 = stock outperforming sector = institutional accumulation
    Returns None if insufficient data (safe default = allow signal)
    Only applied to TREND_PULLBACK and BREAKOUT_PULLBACK.
    NOT applied to RSI_DIVERGENCE or OVERSOLD_EXHAUSTION (reversal setups).
    Test cases: 28/28 passed before implementation.
    """
    if sector_df is None:
        return None  # safe default — missing data never blocks signal

    sd = stock_df[stock_df.index <= dt]
    sec = sector_df[sector_df.index <= dt]

    if len(sd) < RS_LOOKBACK + 1 or len(sec) < RS_LOOKBACK + 1:
        return None  # insufficient history — safe default allow

    try:
        stock_now   = float(sd["close"].iloc[-1])
        stock_63d   = float(sd["close"].iloc[-RS_LOOKBACK])
        sector_now  = float(sec["close"].iloc[-1])
        sector_63d  = float(sec["close"].iloc[-RS_LOOKBACK])

        if stock_63d <= 0 or sector_63d <= 0:
            return None  # bad data — safe default allow

        stock_ret  = stock_now / stock_63d
        sector_ret = sector_now / sector_63d
        ...
        return None  # any error → safe default allow

def _rs_allows(setup, rs_score):
    """
    Gate: RS must be > 1.0 for trend/breakout setups.
    Reversal setups (RSI_DIV, OE) bypass RS gate — they work BECAUSE
    stock has underperformed (been sold off). RS gate would kill them.
    None RS score = missing data = always allow (safe default).
    """
    if setup in ("RSI_DIVERGENCE", "OVERSOLD_EXHAUSTION"):
        return True  # no RS gate for reversal setups
    if rs_score is None:
        return True  # missing data → allow
    return rs_score > 1.0  # strictly above 1.0
...
def run_backtest():
    print("\n"+"="*60)
    print("  BACKTEST V5 — V4 + Relative Strength Filter")
    ...
    for ticker,ddf in all_daily.items():
        ...
        for di in range(250,len(dates)):
            ...
            # Sector gate — hard block for trend/breakout setups (V4)
            if not sector_up and setup in ("TREND_PULLBACK","BREAKOUT_PULLBACK"):
                continue

            # Relative Strength gate — V5 addition
            # Only for trend/breakout setups, not reversal setups
            rs_score = _relative_strength(ddf_to, s_data, dt)
            if not _rs_allows(setup, rs_score):
                continue
            ...
            trades.append({
                ...
                "setup_type":setup,"sector":sector,"sector_uptrend":sector_up,"rs_score":round(rs_score,3) if rs_score else None,
...
def _print_results(df):
    ...
    print(f"\n  COMPARISON")
    print(f"  {'Metric':<22} {'Old':>8} {'V3':>8} {'V4':>8} {'V5':>8}")
    print(f"  {'-'*58}")
    print(f"  {'Win Rate':<22} {'52.1%':>8} {'51.8%':>8} {'54.6%':>8} {wr:.1f}%")
    print(f"  {'Monthly avg':<22} {'6,090':>8} {'5,781':>8} {'9,538':>8} {tot/(yrs*12):,.0f}")
    print(f"  {'Annual return':<22} {'18.4%':>8} {'11.6%':>8} {'19.1%':>8} {tot/CAPITAL/yrs*100:.1f}%")
    print(f"  {'Max drawdown':<22} {'4.2%':>8} {'12.0%':>8} {'5.2%':>8} {mdd*100:.1f}%")
    print(f"  {'Profit factor':<22} {'1.96x':>8} {'1.24x':>8} {'1.45x':>8} {pf:.2f}x")
    ...
    # V5 verdict: must beat V4 on key metrics
    beats_old = sum([wr>52.1, tot/(yrs*12)>6090, mdd*100<4.2, pf>1.96])
    beats_v4  = sum([wr>54.6, tot/(yrs*12)>9538, mdd*100<5.2, pf>1.45])
    print(f"\n  VERDICT vs Old: {beats_old}/4 metrics beaten")
    print(f"  VERDICT vs V4:  {beats_v4}/4 metrics beaten")
    # V5 invalidation criteria (from test cases)
    invalid = []
    if wr < 54.6: invalid.append(f"WR {wr:.1f}% < V4 54.6%")
    if tot/(yrs*12) < 9538: invalid.append(f"Monthly Rs {tot/(yrs*12):,.0f} < V4 Rs 9,538")
    if mdd*100 > 8.0: invalid.append(f"Drawdown {mdd*100:.1f}% > 8% limit")
    if len(df)/yrs < 50: invalid.append(f"Only {len(df)/yrs:.0f} trades/yr < 50 minimum")
    if invalid:
        print(f"  ⚠ V5 INVALIDATED:")
        for i in invalid: print(f"    - {i}")
    else:
        print(f"  ✓ V5 VALID — all invalidation criteria clear")
    print("="*60)
