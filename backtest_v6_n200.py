"""
backtest_v6n200_n200.py - V6 strategy on Nifty 200 universe.

V6 parameters (all locked):
  ATR_SL_MULT=2.0, ATR_TGT_MULT=3.0, ATR_PARTIAL=2.0
  MIN_RR=1.5, HOLD_DAYS=8, RISK=6000, MAX_POS=150000
  OVERSOLD_RSI=38, sector proxies fixed, dup block active

Change from V6: watchlist expanded from 45 to ~130 stocks
Sectors added: NBFC, PSU (new), all existing sectors expanded
Excluded: TELECOM (29% WR), HOSPITAL (41% WR)

Purpose: more trades = more statistically robust results
Compare vs V6: 45 stocks Rs 9,203/mo
"""
...
WATCHLIST = [
    ...
    "LICHSGFIN.NS","SUNDARMFIN.NS","M&MFIN.NS",
    "SHRIRAMFIN.NS","PIRAMALENT.NS",
    # PSU (7 — new sector)
    "RVNL.NS","IRFC.NS","PFC.NS","RECLTD.NS",
    "HUDCO.NS","NBCC.NS","RAILTEL.NS",
]
...
SECTOR_MAP = {
    ...
    "LICHSGFIN.NS":"NBFC","SUNDARMFIN.NS":"NBFC","M&MFIN.NS":"NBFC",
    "SHRIRAMFIN.NS":"NBFC","PIRAMALENT.NS":"NBFC",
    # PSU (new)
    "RVNL.NS":"PSU","IRFC.NS":"PSU","PFC.NS":"PSU","RECLTD.NS":"PSU",
    "HUDCO.NS":"PSU","NBCC.NS":"PSU","RAILTEL.NS":"PSU",
}
...
def run_backtest():
    ...
    outfile=f"backtest_v6n200_{ts}.csv"
...
def _print_results(df):
    print("\n"+"="*60)
    print("  BACKTEST V6 N200 RESULTS")
    print("="*60)
    ...
    print(f"\n  COMPARISON")
    print(f"  {'Metric':<22} {'V4':>8} {'V6(45)':>8} {'V6(200)':>9}")
    print(f"  {'-'*56}")
    print(f"  {'Win Rate':<22} {'54.6%':>8} {'57.8%':>8} {wr:.1f}%")
    print(f"  {'Monthly avg':<22} {'9,538':>8} {'9,203':>8} {tot/(yrs*12):,.0f}")
    print(f"  {'Annual return':<22} {'19.1%':>8} {'18.4%':>8} {tot/CAPITAL/yrs*100:.1f}%")
    print(f"  {'Max drawdown':<22} {'5.2%':>8} {'4.5%':>8} {mdd*100:.1f}%")
    print(f"  {'Profit factor':<22} {'1.45x':>8} {'1.48x':>8} {pf:.2f}x")
    ...
    if tot/(yrs*12)<9203: invalid.append(f"Monthly Rs {tot/(yrs*12):,.0f} < V6(45) Rs 9,203")
