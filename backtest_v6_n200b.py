"""
backtest_v6_n200b.py - V6(N200) + 3 targeted fixes.

V6(N200) base preserved exactly.

Three fixes (test cases 30/30 passed before build):
  1. OVERSOLD_EXHAUSTION restricted to Nifty 50 stocks only
     Midcap OE: 50% WR, Rs -225 avg — pure noise
     Nifty50 OE: 63% WR, Rs +408 avg — genuine signal
  2. Max 6 simultaneous open positions
     Limits correlated crash losses (2025: Rs -155k → target Rs -60k)
  3. Remove 4 weak tickers:
     IRFC (50% WR, Rs -796), RAILTEL (43% WR),
     M&MFIN (44% WR), LICHSGFIN (48% WR, Rs -570)

Expected: Rs 17,000-19,000/month, drawdown 5-6%
Invalidated if: monthly < Rs 15,000, drawdown > 7%, WR < 54%
"""
...
# Max simultaneous open positions — Fix 2
MAX_OPEN = 6

# Nifty 50 tickers — OE restricted to these only — Fix 1
NIFTY50 = {
...
WATCHLIST = [
    ...
    # NBFC (8 — new sector)
    "BAJAJFINSV.NS","CHOLAFIN.NS","MUTHOOTFIN.NS","SUNDARMFIN.NS",# REMOVED (weak WR): "M&MFIN.NS",
    "SHRIRAMFIN.NS","PIRAMALENT.NS",
    # PSU (7 — new sector)
...
def run_backtest():
    ...
    for ticker,ddf in all_daily.items():
        ...
        for di in range(250,len(dates)):
            ...
            if ticker in open_until and di < open_until[ticker]:
                continue
            # Max positions gate -- Fix 2: max 6 simultaneous open
            active = sum(1 for exp in open_until.values() if di < exp)
            if active >= MAX_OPEN:
                continue
    ...
    outfile=f"backtest_v6n200b_{ts}.csv"
...
def _print_results(df):
    print("\n"+"="*60)
    print("  BACKTEST V6 N200b RESULTS")
    print("="*60)
    ...
    print(f"\n  COMPARISON")
    print(f"  {'Metric':<22} {'V6(45)':>8} {'V6(N200)':>10} {'V6(N200b)':>11}")
    print(f"  {'-'*56}")
    print(f"  {'Win Rate':<22} {'57.8%':>8} {'55.5%':>10} {wr:.1f}%")
    print(f"  {'Monthly avg':<22} {'9,203':>8} {'20,013':>10} {tot/(yrs*12):,.0f}")
    print(f"  {'Annual return':<22} {'18.4%':>8} {'40.0%':>10} {tot/CAPITAL/yrs*100:.1f}%")
    print(f"  {'Max drawdown':<22} {'4.5%':>8} {'8.1%':>10} {mdd*100:.1f}%")
    print(f"  {'Profit factor':<22} {'1.48x':>8} {'1.37x':>10} {pf:.2f}x")
    ...
    beats_v6_45  = sum([wr>57.8, tot/(yrs*12)>9203, mdd*100<4.5, pf>1.48])
    beats_n200   = sum([wr>55.5, tot/(yrs*12)>20013, mdd*100<8.1, pf>1.37])
    print(f"\n  VERDICT vs V4(45):  {beats_v4}/4 beaten")
    print(f"  VERDICT vs V6(45):  {beats_v6}/4 beaten")
    invalid=[]
    if wr<54.0: invalid.append(f"WR {wr:.1f}% < 54% minimum")
    if tot/(yrs*12)<15000: invalid.append(f"Monthly Rs {tot/(yrs*12):,.0f} < Rs 15,000 minimum")
    if mdd*100>7.0: invalid.append(f"Drawdown {mdd*100:.1f}% > 7% limit")
